(function($) {
    ...
    passwordInput.oninput= function(){
		...
		if(passwordValue != ""){
			...
			if(whitespace)
			{
				passwordInfo.textContent = "không được nhập khoảng trắng";
			}
			else {
		...
		function poorPasswordStrength(passwordLength, poorPassword, weakPassword, strongPassword){
			if(passwordLength < 8)
			{
			   ...
			   passwordInfo.innerHTML  = "Mật khẩu yếu. Phải chứa ít nhất 8 ký tự";
		...
		function weakPasswordStrength(passwordLength, poorPassword, weakPassword, strongPassword){
			if(passwordLength >= 8 && (poorPassword || weakPassword || strongPassword))
			{
			   ...
			   passwordInfo.innerHTML = "Trung bình. Phải chứa ít nhất 1 chữ cái hoặc số";
		   ...
		   if(passwordLength>= 8 && poorPassword && (weakPassword || strongPassword))
			{
			 ...
			 passwordStrength.classList.remove("heavy-active");
			 passwordInfo.innerHTML = "Almost. Phải chứa ký hiệu đặc biệt";
		  ...
		  if(passwordLength >= 8 && (poorPassword && weakPassword) && strongPassword)
			{
			 ...
			 passwordInfo.innerHTML = "Tuyệt vời! Bạn có một mật khẩu an toàn.";
